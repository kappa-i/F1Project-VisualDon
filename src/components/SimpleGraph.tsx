...
const SimpleGraph = ({
  ...
  const getPercentageDifference = (index: number): { percentage: number; isIncrease: boolean } | null => {
    if (!calculatePercentageDifference || index === 0 || !data[index - 1]) return null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showGrid && (
          <g opacity="0.1">
            {(gridLines === "horizontal" || gridLines === "both") &&
              [0, 1, 2, 3, 4].map((i) => (
                <line key={`h-${i}`} x1="40" y1={40 + (i * 320) / 4} x2="760" y2={40 + (i * 320) / 4}
                  stroke="currentColor" strokeWidth={gridLineThickness}
                  strokeDasharray={gridStyle === "dashed" ? "5,5" : gridStyle === "dotted" ? "1,3" : undefined}
                />
              ))}
            {(gridLines === "vertical" || gridLines === "both") &&
              points.map((point, i) => (
                <line key={`v-${i}`} x1={point.x} y1="40" x2={point.x} y2="360"
                  stroke="currentColor" strokeWidth={gridLineThickness}
                  strokeDasharray={gridStyle === "dashed" ? "5,5" : gridStyle === "dotted" ? "1,3" : undefined}
                />
              ))}
        ...
        {gradientFade && (
          <motion.path d={gradientFillPath} fill="url(#line-gradient)"
            initial={{ opacity: 0 }}
        ...
        <motion.path d={pathD} fill="none" stroke={lineColor} strokeWidth={graphLineThickness}
        ...
        {showDots && points.map((point, index) => (
          <g key={index}
            onMouseEnter={() => { setHoveredIndex(index); setTooltipRotation(0); setTooltipOffsetX(0); }}
          ...
          >
            ...
            {dotHoverGlow && hoveredIndex === index && (
              <motion.circle cx={point.x} cy={point.y} r={dotSize * 2} fill={dotColor}
                initial={{ opacity: 0 }} animate={{ opacity: 0.3 }} exit={{ opacity: 0 }}
                transition={{ duration: 0.2 }}
                style={{ filter: "blur(8px)", pointerEvents: "none" }}
              />
            )}
            <motion.circle cx={point.x} cy={point.y} r={dotSize} fill={dotColor} stroke="white" strokeWidth="2"
              style={{ pointerEvents: "none" }}
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: hoveredIndex === index ? 1.5 : 1, opacity: shouldAnimate ? 1 : 0 }}
              transition={{
        ...
        <AnimatePresence>
          {hoveredIndex !== null && points[hoveredIndex] &&
            !(calculatePercentageDifference && hoveredIndex === 0) && (() => (
              <foreignObject key={hoveredIndex}
                x={points[hoveredIndex].x - 75} y={points[hoveredIndex].y - 84}
                width="150" height="84"
                style={{ overflow: "visible", pointerEvents: "none" }}
              >
                <motion.div
                  initial={{ opacity: 0, scale: 0.8, x: 0 }}
                  animate={{ opacity: 1, scale: 1, x: tooltipOffsetX, rotate: tooltipRotation }}
                  exit={{ opacity: 0, scale: 0.8 }}
                  transition={{ duration: 0.15, x: { type: "spring", stiffness: 300, damping: 30 }, rotate: { type: "spring", stiffness: 300, damping: 30 } }}
                  ...
                  <div className="relative">
                    <div className="bg-white dark:bg-[#1a1a1a] text-gray-900 dark:text-gray-100 px-3 py-2 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 whitespace-nowrap">
                      {calculatePercentageDifference && hoveredIndex > 0 ? (() => {
                        ...
                        if (!diff) return <div className="text-sm font-semibold">{points[hoveredIndex].value.toFixed(2)}</div>;
                        return (
                          <div className="flex items-center gap-1.5">
                            {diff.isIncrease ? <TrendingUp className="w-4 h-4 text-green-400" /> : <TrendingDown className="w-4 h-4 text-red-400" />}
                            <span className={`text-sm font-semibold ${diff.isIncrease ? "text-green-400" : "text-red-400"}`}>
                              {diff.isIncrease ? "+" : "-"}{diff.percentage.toFixed(1)}%
                            </span>
                          </div>
                        );
                      })() : (
                        <div className="text-sm font-semibold">{points[hoveredIndex].value.toFixed(2)}</div>
                      )}
                      {data[hoveredIndex].label && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{data[hoveredIndex].label}</div>
                      )}
                    </div>
                    <div className="absolute left-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-white dark:border-t-[#1a1a1a]"
